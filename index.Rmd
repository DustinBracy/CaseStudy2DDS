...
nbArray <- c('HighlySatisfied','OverTime','LowLevel','Unbalanced',"WorkedOver30",'JobInvolvement',"SalaryHike", "NewHire","NewToRole", "AgeUnder40","DueForPromotion","TopPerformer","NoStock","Gender","MaritalStatus","LogIncome")

#nbArray <-c('JobSatisfaction','OverTime','JobLevel','WorkLifeBalance', 'JobInvolvement',"PercentSalaryHike",
 #          "YearsAtCompany","YearsInCurrentRole","TotalWorkingYears","Age","YearsSinceLastPromotion","PerformanceRating","StockOptionLevel",
 #         'BusinessTravel',"Gender",'JobSatisfaction',"EnvironmentSatisfaction")
#
#nbArray <- c('OverTime', 'LowLevel', 'NoStock', 'NewToRole','JobInvolvement')
...
for(j in 1:iterations)
{
  ...
  #fastNaiveBayes(train[,nbArray], as.factor(train$Attrition), laplace = .0001)
  model = naiveBayes(train[,nbArray],as.factor(train$Attrition),laplace = .0001)
  #model = fnb.train(train[,nbArray],as.factor(train$Attrition),laplace = .0001)
