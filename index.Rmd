...
oData %>% select("JobRole",'OverTime','WorkLifeBalance','JobInvolvement',"PercentSalaryHike", "NumCompaniesWorked","BusinessTravel","YearsAtCompany","Education") %>% ggpairs()
...
#LogReg
model3 <- glm(Attrition ~ HighlySatisfied+ OverTime+ LowLevel+Unbalanced+WorkedOver30+JobInvolvement+SalaryHike+ NewHire+NewToRole+ AgeUnder40+DueForPromotion+TopPerformer+NoStock+MaritalStatus+LogIncome+MonthlyOver15k+HourlyOver40, data=train, family="binomial" )
summary(model3)
atPrd <- predict(model3, type='response', newdata = test)
actualPred <- ifelse(atPrd > 0.5, "Yes", "No")
