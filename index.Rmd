...
colors <- c("#20A2DC", "#B7D432", "#EC2029", "#EC2029","#6A757D","#F57A20")
...
```{r EDA Continuous, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center"}
...
```{r EDA Ordinal, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center"}
...
```{r EDA Categorical, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center"}
...
pDept <- oData %>% ggplot(aes(Department, fill=Attrition)) + geom_bar()
...
rdata <- subset(data, select = -c(Over18, Department, JobRole, MaritalStatus, EducationField, EmployeeCount, StandardHours, ID))
...
```{r determine correlation, warning=FALSE, echo=FALSE, message=FALSE, fig.align="center"}
data.cor <- cor(rdata)
...
```{r view correlation, echo=FALSE, message=FALSE, fig.align="center"}
...
confusionMatrix(table(test$Attrition,classifications, dnn = c("Prediction", "Reference")))
...
confusionMatrix(table(predict(model,test[,nbArray]),as.factor(test$Attrition), dnn = c("Prediction", "Reference")))
...
confusionMatrix(table(predict(model2,test2[,nbArray2]),as.factor(test2$Attrition), dnn = c("Prediction", "Reference")))
...
confusionMatrix(table(as.factor(actualPred), as.factor(test$Attrition), dnn = c("Prediction", "Reference")))
